...
export const translations: Record<Lang, Translation> = {
  en: {
    ...
    projects: [
      ...
      { category: 'Business / Management', description: 'A business-management app for shop owners. Its dashboard shows total sales, how much is owed on credit and net profit at a glance, along with your top-selling products. Manage several stores at once — each store’s data stays separate — browse and edit every product in your database, and place purchase orders with your suppliers.' },
      { category: 'Lifestyle / Islamic', description: 'A complete Islamic companion app. Accurate prayer times, a Qibla-direction compass and Azan alerts based on your location, plus a collection of duas and hadiths, a digital tasbih counter, and a nearest-mosques finder. Its prayer challenge lets you take on a 30-day tracker — mark each prayer you complete and watch your completion percentage grow.' },
      ...
      { category: 'Auto / Utility', description: 'An all-in-one app for drivers in Uzbekistan. Scan a car’s QR sticker to reach the owner without sharing a phone number, check your traffic fines, and use the built-in anti-radar — it warns you about speed cameras and, if you go over the limit, shows exactly how much you’d be fined (in UZS) and how many penalty points you’d get.' },
  ...
  ru: {
    ...
    projects: [
      ...
      { category: 'Бизнес / Управление', description: 'Приложение для управления бизнесом для владельцев магазинов. Дашборд с одного взгляда показывает общие продажи, сумму задолженности в кредит и чистую прибыль, а также самые продаваемые товары. Управляйте сразу несколькими магазинами — данные каждого хранятся отдельно, — просматривайте и редактируйте все товары в базе и оформляйте заказы поставщикам.' },
      { category: 'Образ жизни / Ислам', description: 'Полноценное исламское приложение-компаньон. Точное время намаза, компас направления Киблы и уведомления азана по геолокации, а также сборник дуа и хадисов, цифровой счётчик тасбих и поиск ближайших мечетей. Функция намаз-челленджа предлагает 30-дневный трекер — отмечайте каждый совершённый намаз и следите за ростом процента выполнения.' },
      ...
      { category: 'Авто / Утилита', description: 'Многофункциональное приложение для водителей Узбекистана. Отсканируйте QR-наклейку авто, чтобы связаться с владельцем без обмена номером телефона, проверяйте свои штрафы и пользуйтесь встроенным антирадаром — он предупреждает о камерах скорости и, если вы превышаете лимит, показывает, на какую сумму (в сумах) будет штраф и сколько штрафных баллов вы получите.' },
  ...
  uz: {
    ...
    projects: [
      ...
      { category: 'Biznes / Boshqaruv', description: "Do'kon egalari uchun biznes boshqaruv ilovasi. Dashboard bir qarashda umumiy savdo, qarzga berilgan summa va sof foydani, shuningdek eng ko'p sotilgan mahsulotlarni ko'rsatadi. Bir nechta do'konni bir vaqtda boshqaring — har birining ma'lumotlari alohida turadi — bazangizdagi barcha mahsulotlarni ko'ring va tahrirlang hamda ta'minotchilarga zakaz bering." },
      { category: 'Turmush / Islomiy', description: "To'liq islomiy hamroh ilova. Joylashuvingizga qarab aniq namoz vaqtlari, Qibla yo'nalishi kompasi va azon eslatmalari, shuningdek duo va hadislar to'plami, raqamli tasbeh sanagichi va eng yaqin masjidlarni topish. Namoz challenji orqali 30 kunlik trekerni boshlaysiz — o'qigan har bir namozingizni belgilab borasiz va bajarilish foizi oshib borishini kuzatasiz." },
      ...
      { category: 'Auto / Utility', description: "O'zbekiston haydovchilari uchun ko'p vazifali ilova. Mashinaning QR stikerini skanerlab, telefon raqamini ulashmasdan egasi bilan bog'laning, jarimalaringizni ko'ring va o'rnatilgan antiradardan foydalaning — u tezlik kameralari haqida ogohlantiradi va agar tezlikni oshirsangiz, qancha (so'mda) jarima to'lashingiz va nechta jarima ball olishingizni aniq ko'rsatadi." },
